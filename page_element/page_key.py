...
class PageKey():
    ...
    def __init__(self, driver):
        ...
        self.btn_PauseVideo = self.driver(resourceId="com.gdu.pro2:id/btn_PauseVideo")       # 相册图片
        self.videoView = self.driver(resourceId="com.gdu.pro2:id/videoView")                 # 相册视频
    ...
    def photo_video_distinguish(self, name, type="可见光"):
        ...
        try:
            ...
            self.album.click()
            sleep(1)
            if self.btn_PauseVideo.exists():
                self.btn_PauseVideo.screenshot().save(F"{Config.screenshot_path}\\{name}_photo.png")
                if type == "分屏":
                    screenshot_png(F"{Config.screenshot_path}\\{name}_photo.png")
                text = erweima_function(F"{Config.screenshot_path}\\{name}_photo.png")
                mylog.info(F"*****{text}*****")
                self.del_picture()
                return text
            else:
