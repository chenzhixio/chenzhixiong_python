...
class test_Yuntai_adjustment(PageKey):
    ...
    def set_ev(self, name, element, num):
        ...
        element.click()
        sleep(3)
        distinguish = self.photo_video_distinguish(name)
        self.operation_load(1)
        sleep(6)
        text = element.info.get('selected')
        mylog.info(F"{num}是否为选中状态：{text}-------{distinguish}")
        return text, distinguish
    ...
    def Yuntai_adjustment_005(self):
        try:
            ...
            text_list = []
            for ev in values:
                text, distinguish = self.set_ev("Yuntai_adjustment_004", ev[0], ev[1])
    ...
    def Yuntai_adjustment_006(self):
        try:
            ...
            text_list = []
            for ev in values:
                text, distinguish = self.set_ev("Yuntai_adjustment_005", ev[0], ev[1])
                distinguish_list.append(distinguish)
                text_list.append(text)
            self.driver.click(self.x, self.y)
            return text_list, distinguish_list
    ...
    def Yuntai_adjustment_007(self):
        try:
            ...
            text_list = []
            for ev in values:
                text, distinguish = self.set_ev("Yuntai_adjustment_007", ev[0], ev[1])
                distinguish_list.append(distinguish)
                text_list.append(text)
            self.driver.click(self.x, self.y)
            sleep(1)
    ...
    def Yuntai_adjustment_008(self):
        try:
            ...
            text_list = []
            for ev in values:
                text, distinguish = self.set_ev("Yuntai_adjustment_006", ev[0], ev[1])
                distinguish_list.append(distinguish)
                text_list.append(text)
            self.driver.click(self.x, self.y)
            return text_list, distinguish_list
    ...
    def Yuntai_adjustment_009(self):
        try:
            ...
            photo1 = self.photo_video_distinguish("Yuntai_adjustment_008_001")
            self.driver.click(self.x, self.y)
            mylog.info(F"设置红外亮度为：{text1}---------出图情况：{photo1}")
            sleep(2)
            self.operation_load(1)
            self.driver.click(1784, 642)
            sleep(5)
            text2 = self.driver(resourceId="com.gdu.pro2:id/tv_irda_setting_brightness").get_text()
            self.driver.click(1500, 642)
            sleep(2)
            photo2 = self.photo_video_distinguish("Yuntai_adjustment_008_002")
            self.driver.click(self.x, self.y)
    ...
    def Yuntai_adjustment_010(self):
        try:
            self.set_type("红外")
            self.operation_load(1)
            self.driver.click(1298, 790)
            sleep(5)
            text1 = self.driver(resourceId="com.gdu.pro2:id/tv_irda_setting_contrast").get_text()
            photo1 = self.photo_video_distinguish("Yuntai_adjustment_009_001")
            self.driver.click(self.x, self.y)
            mylog.info(F"设置红外亮度为：{text1}---------出图情况：{photo1}")
            sleep(2)
            self.operation_load(1)
            self.driver.click(1798, 790)
            sleep(5)
            text2 = self.driver(resourceId="com.gdu.pro2:id/tv_irda_setting_contrast").get_text()
            photo2 = self.photo_video_distinguish("Yuntai_adjustment_009_002")
